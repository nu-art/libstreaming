...
public class RtspClient {
	...
	private void sendRequestAnnounce()
		...
		String request = "ANNOUNCE rtsp://" + mParameters.host + ":" + mParameters.port + mParameters.path + " RTSP/1.0" + LineBreak + "CSeq: " + (++mCSeq) + LineBreak + "Content-Length: " + body
				.length() + LineBreak + "Content-Type: application/sdp" + LineBreak + LineBreak + body;
		Log.i(TAG, request.substring(0, request.indexOf(LineBreak)));
		...
		if (response.status == 401) {
			...
			request = "ANNOUNCE rtsp://" + mParameters.host + ":" + mParameters.port + mParameters.path + " RTSP/1.0" + LineBreak + "CSeq: " + (++mCSeq) + LineBreak + "Content-Length: " + body
					.length() + LineBreak + "Authorization: " + mAuthorization + LineBreak + "Session: " + mSessionID + LineBreak + "Content-Type: application/sdp\r\n" + LineBreak + body;
	...
	private void sendRequestSetup()
		...
		for (int i = 0; i < 2; i++) {
			...
			if (stream != null) {
				...
				String request = "SETUP rtsp://" + mParameters.host + ":" + mParameters.port + mParameters.path + "/trackID=" + i + " RTSP/1.0" + LineBreak + "Transport: RTP/AVP/" + params + LineBreak + addHeaders();
	...
	private void sendRequestRecord()
			throws IllegalStateException, SocketException, IOException {
		String request = "RECORD rtsp://" + mParameters.host + ":" + mParameters.port + mParameters.path + " RTSP/1.0" + LineBreak + "Range: npt=0.000-" + LineBreak + addHeaders();
		Log.i(TAG, request.substring(0, request.indexOf(LineBreak)));
		mOutputStream.write(request.getBytes("UTF-8"));
		mOutputStream.flush();
	...
	private void sendRequestTeardown()
			throws IOException {
		String request = "TEARDOWN rtsp://" + mParameters.host + ":" + mParameters.port + mParameters.path + " RTSP/1.0" + LineBreak + addHeaders();
		Log.i(TAG, request.substring(0, request.indexOf(LineBreak)));
		mOutputStream.write(request.getBytes("UTF-8"));
		mOutputStream.flush();
	}
	...
	private void sendRequestOption()
			throws IOException {
		String request = "OPTIONS rtsp://" + mParameters.host + ":" + mParameters.port + mParameters.path + " RTSP/1.0" + LineBreak + addHeaders();
		Log.i(TAG, request.substring(0, request.indexOf(LineBreak)));
		mOutputStream.write(request.getBytes("UTF-8"));
		mOutputStream.flush();
		Response.parseResponse(mBufferedReader);
	...
	private String addHeaders() {
		return "CSeq: " + (++mCSeq) + LineBreak + "Content-Length: 0" + LineBreak + "Session: " + mSessionID + LineBreak +
				// For some reason you may have to remove last LineBreak in the next line to make the RTSP client work with your wowza server :/
				(mAuthorization != null ? "Authorization: " + mAuthorization + LineBreak : "") + LineBreak;
	}
